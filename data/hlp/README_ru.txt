...
Можно даже по-разному трактовать и само название проги expagog, но педагогическая составляющая в нем безусловно есть.
...
Но даже не очень умелые учителя в силу своей профессии понуждают нас к добру. Примерно так же можно использовать и EG - когда умело, когда не очень, но всегда в направлении добра. И никакого сектантства! Какое там еще сектантство в школе?
...
"Тема" - любое занятие, тема или тип эксперимента / приключения, например: "НЕ_Курю" (пробелы в названиях тем исключаются).
...
"Формат" - задает тип значений темы; возможные форматы: число ("99.9"), время ("time"), переключатель ("chk"), текст ("xxx").
...
"Комментарий к ячейке" - произвольный текст для текущей ячейки; может содержать теги; комментарии можно искать с помощью функции Find.

"Общие заметки" - произвольный текст под диаграммой, содержащий замечания и выводы по текущей базе, диаграмме и пр.

"Стикер" - краткая заметка, которая может пригодиться в работе с EG, например, критерии и эквиваленты оценок; стикеры можно помещать в любом месте экрана и раскрашивать любым цветом.
...
"Счетчики Count / Count0" - в статистике означают общее число непустых ячеек и число непустых ячеек со значением 0.
...
Для изменения настроек кликните кнопку меню ("гамбургер") и выберите пункт Settings.
...
Особое внимание обратите на тему "EG" - это общая оценка прожитого дня. Определите для нее шкалу раз и навсегда, например, 5-, 10- или 100-бальную. Критерии оценок можете вынести в стикер, оставив его открытым в удобном для вас месте экрана.
...
Диалог Find после открытия можно не закрывать, он не влияет на работу в основном окне EG. Геометрия и опции диалога сохраняются до следующей сессии.


 ДИАГРАММЫ
===========

При старте EG отображается гистограмма для агрегированного показателя AggrEG, его формула находится над графиком. Формулу можно исправить в любое время, подтвердив изменение нажатием клавиши Enter. В формуле можно указывать названия тем, например: Dist/Time.
...
Показатель Totals означает сумму всех значений всех тем, независимо от их "веса", отражаемого в AggrEG.
...
Для обновления текущей диаграммы (при изменении исходных данных) можно кликнуть кнопку "Redraw" в инструментах диаграммы или нажать клавишу F5. Прокрутить диаграмму влево-вправо можно соответствующими кнопками.
...
Диалог Statistics после открытия можно не закрывать, он не влияет на работу в основном окне EG. Геометрия и опции диалога сохраняются до следующей сессии.
...
Дополнительные:
 ...
 JS code      - JavaScript code to be executed at opening
                the report by a browser

Остальные настройки также можно задать, они отражаются в заголовке отчета:
...
Для изменения настроек текущей базы надо выбрать пункт Settings в меню.

Основные настройки это список тегов и список тем.
...
Типы данных:
 ...
 - time   : тип "время", в таблице вводится как чч.мм (часы.минуты),
            отображается как чч:мм или
 - chk    : переключатель между "?", "да", "лампа", "нет",
 -          означающими значения 0, 2, 1, -1 соответственно
 - calc   : вычисляемый тип
 - прочие : тип "произвольный текст", возможно с ведущими знаками "+" / "-"
            для повышения / понижения его значения
...
Можно также переименовывать существующие темы. Названия темам желательно давать краткие. Пробелы в названиях исключаются.
...
Учтите следующее:
 ...
 2) Режим автосохранения архива хорошо включать на стационарном компе, где и флешка торчит в своем гнезде более-менее стационарно. С ноутбуком это не прокатит.

 3) Для сохранения данных во время работы с EG нажмите кнопку Save в панели инструментов EG, либо выберите пункт Save из меню, либо нажмите комбинацию клавиш Ctrl+S.
...
Исходники:
 ...
 https://chiselapp.com/user/aplsimple/repository/expagog
 https://github.com/aplsimple/expagog

Установщики:

 https://github.com/aplsimple/expagog/releases/tag/Installers_of_EG-1.0

Демо-ролик:

 https://github.com/aplsimple/expagog/releases/tag/Demos_of_EG-1.0

Скриншоты:

 https://github.com/aplsimple/expagog/releases/tag/Screens_of_EG-1.0
